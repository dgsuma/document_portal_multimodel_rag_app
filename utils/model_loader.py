...
class ModelLoader:
    ...
    def _validate_env(self):
        ...
        """
        required_vars = ["GOOGLE_API_KEY", "GROQ_API_KEY", "TOGETHER_API_KEY"]  # add new keys
        self.api_keys = {key: os.getenv(key) for key in required_vars}
        missing = [k for k, v in self.api_keys.items() if not v and k != "TOGETHER_API_KEY"]
        if missing:
    ...
    def load_llm(self):
        ...
        #provider_key = os.getenv("LLM_PROVIDER", "together")  # default together
...
#         self.api_keys={key:os.getenv(key) for key in required_vars} # dictionary comprehension to get env vars
